...
object:
	...
	avr-gcc $(cflags) -mmcu=$(avrType) -Wa,-ahlmns=$(src).lst -c -o uart.o uart.c

elf: object
	avr-gcc $(cflags) -mmcu=$(avrType) -flto -o $(src).elf $(src).o uart.o
...
eeprom: hex
	avrdude -p$(avrType) -c$(programmerType) -P$(programmerDev) -b$(baud) -B$(B) -v -U eeprom:w:$(src).eeprom.hex
	date
...
flash: hex
	avrdude -p$(avrType) -c$(programmerType) -P$(programmerDev) -b$(baud) -B$(B) -v -U flash:w:$(src).flash.hex

backup:
	@for memory in $(memoryTypes); do \
	    avrdude -p $(avrType) -c$(programmerType) -P$(programmerDev) -b$(baud) -B$(B) -v -U $$memory:r:./$(avrType).$$memory.hex:i; \
	done
